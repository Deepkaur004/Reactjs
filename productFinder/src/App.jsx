...
function ProductTable({ products, filterText, inStockOnly }) {
  ...
  products.forEach((product) => {
    ...
    if (product.category != lastCategory) {
      rows.push(
        <ProductCategoryRow category={product.category} key={product.key} />
      )
    }
...
function SearchBar({filterText, inStockOnly, onFilterTextChange, onInStockOnlyChange}) {
  return (
    <form>
      <input type='text' value={filterText}  placeholder='search a product' onChange={(e) => onFilterTextChange(e.target.value)} />
      <label>
